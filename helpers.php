...
function fw_ext_photography_get_gallery_images( $post_id = 0 ) {
	if ( 0 === $post_id && null === ( $post_id = get_the_ID() ) ) {
		return array();
	}

	return fw_get_db_post_option( $post_id, 'photography-gallery', array() );
}
 ...
 */
function fw_ext_photography_get_listing_categories( $term_ids ) {
	...
	$ext_photography_settings = fw()->extensions->get( 'photography' )->get_settings();
	$taxonomy                 = $ext_photography_settings['taxonomy_name'];
 ...
 * @return array
 */
function fw_ext_photography_get_sort_classes( array $items, array $categories, $prefix = 'category_' ) {
